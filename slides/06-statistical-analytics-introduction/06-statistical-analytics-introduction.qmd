...
| :------- | :------------- | :------------- | :------------------- |
| 二项分布 | `binom.pdf`    | `binom.cdf`    | `binom.rvs`          |
| 几何分布 | `geom.pdf`     | `geom.cdf`     | `geom.rvs`           |
| 泊松分布 | `poisson.pdf`  | `poisson.cdf`  | `poisson.rvs`        |
| 均匀分布 | `uniform.pdf`  | `uniform.cdf`  | `uniform.rvs`        |
| 正态分布 | `norm.pdf`     | `norm.cdf`     | `norm.rvs`           |
| 指数分布 | `expon.pdf`    | `expon.cdf`    | `expon.rvs`          |
...
![](images/size-price-ln-model.png){fig-align="center" width=100%}

## 一元线性回归

利用 `statsmodels.regression.linear_model.OLS` 或 `statsmodels.formula.api.ols` 进行线性模型拟合，`OLS` 和 `ols` 定义如下：
...
在模型函数表达式（formula）类似 R 语言，除了会用到操作符外，还会用到一些特殊的符号和函数。表达式常用的符号和函数如下 ^[<https://patsy.readthedocs.io/en/latest/formulas.html>]：
