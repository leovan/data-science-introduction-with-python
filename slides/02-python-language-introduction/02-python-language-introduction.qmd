...
Python ^[<https://zh.wikipedia.org/wiki/Python>] 是由 Guido van Rossum ^[<https://zh.wikipedia.org/wiki/吉多·范罗苏姆>] 于 1991 年创造的一种广泛使用的解释型、高级编程、通用型编程语言。
...
对象的拷贝分为浅层拷贝和深层拷贝，两个的具体区别请参见官方文档 ^[浅层和深层拷贝：<https://docs.python.org/zh-cn/3/library/copy.html>]。
:::
::::
...
```{python}
'Hello, {0}. You are {1}, {0}.'.format(name, age)
```
:::
::::
...
::: aside
格式化参考：<https://docs.python.org/zh-cn/3/library/string.html>
:::
...
内建的 `defaultdict` 可以向字典中传入类型或能在各个位置生成默认值的函数：
...
:::: {.columns}
::: {.column width="50%"}
集合是一种无序且元素唯一的容器，集合可以通过两种方式创建：通过 `set` 函数创建或用字面值集合大括号语法创建：
...
|                     |          |                         | `a.symmetric_difference`<br/>`_update(b)` | `a ^= b`  | 将 `a` 置为 `a` 和 `b` 的对称差集 |

: {tbl-colwidths="[16,10,16,23,10,25]"}
...
::: {.callout-note title="不要在行尾加分号, 也不要用分号将两条命令放在同一行。"}
:::

::: {.callout-note title="每行不超过 80 个字符。"}
:::
...
`# http://www.example.com/us/developer/documentation/api/content/\`<br/>
...
::: {.callout-note title="顶级定义之间空两行，方法定义之间空一行。"}
:::
...
:::: {.columns}
::: {.column width="50%"}
::: {.callout-note title="用 4 个空格来缩进代码。"}
:::
...
:::: {.columns}
::: {.column width="50%"}
::: {.callout-note title="按照标准的排版规范来使用标点两边的空格。"}
:::
...
::: {.callout-note title="参数列表，索引或切片的左括号前不应加空格。"}
:::
...
::: {.callout-note title="大部分 `.py` 文件不必以 `#!` 开始。根据 PEP-394 ，程序的 `main` 文件应该以 `#!/usr/bin/python2` 或者 `#!/usr/bin/python3` 开始。"}
:::
...
::: {.callout-note title="确保对模块，函数，方法和行内注释使用正确的风格。"}
:::
...
::: {.callout-note title="即使参数都是字符串，使用 `%` 或者格式化方法格式化字符串。不过也不能一概而论，你需要在 `+` 和 `%` 之间好好判定。"}
:::
...
::: {.callout-note title="为临时代码使用 TODO 注释，它是一种短期解决方案，不算完美，但够好了。"}
:::
...
::: {.callout-note title="通常每个语句应该独占一行。"}
:::
...
::: {.callout-note title="在 Python 中，对于琐碎又不太重要的访问函数，你应该直接使用公有变量来取代它们，这样可以避免额外的函数调用开销。当添加更多功能时，你可以用属性（property）来保持语法的一致性。"}
:::
...
::: {.callout-note title="`module_name, package_name, ClassName, method_name, ExceptionName, function_name, GLOBAL_VAR_NAME, instance_var_name, function_parameter_name, local_var_name`"}
:::
...
::: {.callout-note title="即使是一个打算被用作脚本的文件，也应该是可导入的。并且简单的导入不应该导致这个脚本的主功能被执行，这是一种副作用，主功能应该放在一个 `main()` 函数中。"}
:::
